...
function parseBooleanField(body, fieldName, fallback = false) {
  if (!body || typeof body !== "object") {
    return fallback;
  }

  const matchedKey = Object.keys(body).find(
  ...
  const rawValue = Array.isArray(body[matchedKey]) ? body[matchedKey][0] : body[matchedKey];
...
function parseProductFields(body, options = {}) {
  return {
    name: (body.name || "").trim(),
    ...
    isOnSale: parseBooleanField(body, "isOnSale", Boolean(options.defaultIsOnSale)),
  };
}
  ...
  upload.single("image")(req, res, async (uploadError) => {
    ...
    try {
      ...
      const productData = parseProductFields(req.body, {
        defaultIsOnSale: existingProduct.isOnSale,
      });//proper values
      ...
      existingProduct.isOnSale = productData.isOnSale;
