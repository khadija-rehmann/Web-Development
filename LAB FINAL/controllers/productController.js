...
// Return a plain product object with consistent pricing fields for the views.
function withSalePricing(productDoc) {
  const product = typeof productDoc.toObject === "function" ? productDoc.toObject() : { ...productDoc };
  const basePrice = Number(product.price) || 0;

  // Keep the base/original price so views can show a strike-through value.
  product.originalPrice = basePrice;

  if (product.isOnSale) {
    const hasValidSalePrice = Number(product.salePrice) > 0 && Number(product.salePrice) < basePrice;
    const computedSalePrice = Math.round(basePrice * 0.8);

    // Use DB salePrice when valid, otherwise fallback to a simple 20% off value.
    product.salePrice = hasValidSalePrice ? Number(product.salePrice) : computedSalePrice;
...
exports.homepage = async (req, res) => {
  try {
    ...
    const newArrivalsRaw = await Product.find({ isNewArrival: true }).limit(8);
    const newArrivals = newArrivalsRaw.map(withSalePricing);
...
exports.women = async (req, res) => {
  try {
    ...
    const productsRaw = await Product.find({ category: { $in: womenCategories } });
    const products = productsRaw.map(withSalePricing);
...
exports.men = async (req, res) => {
  try {
    const Product = getProduct();
    const productsRaw = await Product.find({ category: "Men" });
    const products = productsRaw.map(withSalePricing);
...
exports.girls = async (req, res) => {
  try {
    const Product = getProduct();
    const productsRaw = await Product.find({ category: "Girls" });
    const products = productsRaw.map(withSalePricing);
...
exports.unstitched = async (req, res) => {
  try {
    const Product = getProduct();
    const productsRaw = await Product.find({ category: 'Unstitched' });
    const products = productsRaw.map(withSalePricing);
...
exports.readyToWear = async (req, res) => {
  try {
    const Product = getProduct();
    const productsRaw = await Product.find({ category: 'Ready to Wear' });
    const products = productsRaw.map(withSalePricing);
...
exports.fragrances = async (req, res) => {
  try {
    const Product = getProduct();
    const productsRaw = await Product.find({ category: 'Fragrances' });
    const products = productsRaw.map(withSalePricing);
...
exports.bags = async (req, res) => {
  try {
    const Product = getProduct();
    const productsRaw = await Product.find({ category: 'Bags' });
    const products = productsRaw.map(withSalePricing);
...
exports.footwear = async (req, res) => {
  try {
    const Product = getProduct();
    const productsRaw = await Product.find({ category: 'Footwear' });
    const products = productsRaw.map(withSalePricing);
...
// Render on-sale products page
exports.onsale = async (req, res) => {
  try {
    const Product = getProduct(); // lazy-require the Product model

    // Find all products where isOnSale is true
    const productsRaw = await Product.find({ isOnSale: true });
    const products = productsRaw.map(withSalePricing);

    // Render the onsale.ejs view and pass the products array
    res.render('onsale', { products });
  } catch (err) {
    console.error('Error fetching on-sale products:', err);
    // On error, render the onsale page with an empty list to show friendly message
    res.render('onsale', { products: [] });
  }
};